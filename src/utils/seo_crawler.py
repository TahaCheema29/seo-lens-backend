...
class SEOCrawler:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for line in lines:
                    ...
                    if line.lower().startswith("user-agent:"):
                    ...
                    elif line.lower().startswith("allow:") and current_user_agent in (
                        "*",
                        "seo-lens-bot",
                    ):
                        ...
                        "disallow:"
                    ) and current_user_agent in ("*", "seo-lens-bot"):
    ...
    async def get_urls_from_sitemap(
        self, sitemap_urls: str | set[str], visited=None
    ) -> list[str]:
        ...
        for sitemap_url in sitemap_urls:
            try:
                ...
                print(
                    f"[!] Requests failed for {sitemap_url}: {e} — retrying with Playwright..."
                )
                try:
            ...
            try:
                ...
                for sub_sitemap in root.xpath(
                    ...
                    if sub_url not in visited:
                        visited.add(sub_url)
                        urls.extend(
                            await self.get_urls_from_sitemap({sub_url}, visited)
                        )
            except Exception as e:
    ...
    async def run(self):
        ...
        if self.crawl_mode == CrawlMode.FULL_CRAWL:
            ...
            for url in all_urls:
                if self.is_allowed(url, disallowed_paths):
                    ...
                    self.url_info[url] = URLStatus.BLOCKED
            print("len of url is ", len(all_urls))
            if len(all_urls) == 0:
                print("is allowed ", self.is_allowed(self.start_url, disallowed_paths))
        ...
        print("\n✅ Crawl complete")
        print(
            f"Total URLs tracked: {len(self.url_info) if self.crawl_mode == CrawlMode.FULL_CRAWL else len(all_urls)}"
        )
